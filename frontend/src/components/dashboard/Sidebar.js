...
function Sidebar({ mobileOpen = false, onCloseMobile }) {
    ...
    const content = (<aside className="w-60 shrink-0 bg-navy flex flex-col h-full overflow-y-auto z-40">
      {/* Logo & Mobile Close */}
      <div className="px-5 pt-5 pb-4 border-b border-white/10 flex items-center justify-between">
        ...
        {onCloseMobile && (<button onClick={onCloseMobile} className="lg:hidden text-white/60 hover:text-white p-1">
            ✕
          </button>)}
      </div>
      ...
      <div className="mx-3 mt-3 bg-white/[0.07] border border-white/10 rounded-xl px-3 py-2.5">
        ...
        <div className="flex gap-1.5">
          {PLATFORM_STATUSES.map((p) => (<div key={p.letter} className="relative group">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white font-black text-xs" style={{ background: p.color, opacity: p.ok ? 1 : 0.45 }}>
                {p.letter}
              </div>
              {!p.ok && (<div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-red-500 rounded-full border border-navy"/>)}
            </div>))}
        </div>
            ...
            <ul className="space-y-0.5">
              {section.items.map((item) => {
                ...
                return (<li key={item.href}>
                    <a href={item.href} onClick={onCloseMobile} className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-semibold transition-all group ${active
