    ...
    return (<footer className="bg-navy text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 mb-14">
          ...
          {/* Links */}
          {Object.entries(LINKS).map(([group, items]) => (<div key={group}>
        ...
        <div className="border-t border-white/10 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5">
            {['Amazon', 'Flipkart', 'Meesho', 'Shopify'].map((p) => (<span key={p} className="text-white/30 text-xs hover:text-white/60 cursor-pointer transition-colors">
                {p}
              </span>))}
          </div>
        </div>
