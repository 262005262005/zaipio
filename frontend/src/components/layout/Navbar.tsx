...
export default function Navbar() {
  ...
  const [mobileAccordion, setMobileAccordion] = useState<'product' | 'solutions' | 'resources' | null>(null)
  ...
  return (
    <header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled ? 'bg-navy/95 backdrop-blur-md shadow-navy-md border-b border-white/10' : 'bg-navy/80 backdrop-blur-sm border-b border-white/10'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-[88px]">
          ...
          <nav className="hidden md:flex items-center gap-3">
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-base font-bold transition-all ${
                  activeDropdown === 'product' ? 'bg-white/15 text-white shadow-sm' : 'text-white/90 hover:bg-white/10 hover:text-white'
                }`}
              >
                <span>Product</span>
              ...
              {activeDropdown === 'product' && (
                <div className="absolute top-full left-0 mt-3 w-[760px] bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 grid grid-cols-3 gap-6 animate-fade-in z-50 text-left">
                  {PRODUCT_MENU.map((col) => (
                    <div key={col.group} className="space-y-3">
                      ...
                      <div className="space-y-2">
                        {col.items.map((item) => (
                          <a
                            ...
                            className="block p-3 rounded-xl hover:bg-slate-800 transition-all group"
                          >
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-base font-bold transition-all ${
                  activeDropdown === 'solutions' ? 'bg-white/15 text-white shadow-sm' : 'text-white/90 hover:bg-white/10 hover:text-white'
                }`}
              >
                <span>Solutions</span>
              ...
              {activeDropdown === 'solutions' && (
                <div className="absolute top-full left-0 mt-3 w-[540px] bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 grid grid-cols-2 gap-6 animate-fade-in z-50 text-left">
                      ...
                      <div className="space-y-2">
                        {col.items.map((item) => (
                          <a
                            ...
                            href={item.href}
                            onClick={() => setActiveDropdown(null)}
                            className="flex items-start gap-3 p-2.5 rounded-xl hover:bg-slate-800 transition-all group"
                          >
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-base font-bold transition-all ${
                  activeDropdown === 'resources' ? 'bg-white/15 text-white shadow-sm' : 'text-white/90 hover:bg-white/10 hover:text-white'
                }`}
              >
                <span>Resources</span>
              ...
              {activeDropdown === 'resources' && (
                <div className="absolute top-full left-0 mt-3 w-[340px] bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-4 space-y-2 animate-fade-in z-50 text-left">
          ...
          <div className="hidden md:flex items-center gap-5">
            ...
            <a
              href="/signup"
              className="bg-gradient-to-r from-accent to-sky-600 text-white font-black text-sm px-6 py-3 rounded-xl shadow-[0_0_24px_rgba(14,165,233,0.5)] hover:brightness-110 hover:-translate-y-0.5 transition-all"
            >
              Start Free →
            </a>
          </div>
          ...
          <button
            ...
            className="md:hidden p-2.5 rounded-xl bg-white/10 text-white font-bold text-xl"
          >
            {mobileMenuOpen ? '✕' : '☰'}
          </button>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-slate-950 border-b border-slate-800 p-6 space-y-4 text-left max-h-[85vh] overflow-y-auto">

          {/* Product Accordion */}
          <div>
            <button
              onClick={() => setMobileAccordion(mobileAccordion === 'product' ? null : 'product')}
              className="flex items-center justify-between w-full text-white font-bold text-base py-2.5 border-b border-slate-800/80"
            >
              <span>Product</span>
              <span className="text-xs">{mobileAccordion === 'product' ? '▲' : '▼'}</span>
            </button>
            {mobileAccordion === 'product' && (
              <div className="py-2 space-y-2 pl-3">
                {PRODUCT_MENU.flatMap(g => g.items).map((item) => (
                  <a key={item.title} href={item.href} onClick={() => setMobileMenuOpen(false)} className="block py-1.5">
          ...
          <div>
            ...
            {mobileAccordion === 'solutions' && (
              <div className="py-2 space-y-2 pl-3">
                {SOLUTIONS_MENU.flatMap(g => g.items).map((item) => (
                  <a key={item.title} href={item.href} onClick={() => setMobileMenuOpen(false)} className="block py-1.5">
