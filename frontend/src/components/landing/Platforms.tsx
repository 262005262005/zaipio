...
export default function Platforms() {
  return (
    <section id="platforms" className="py-24 bg-navy/[0.02] border-y border-navy/10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {PLATFORMS.map((p) => (
            <div
              key={p.name}
              className="bg-white rounded-2xl border border-navy/10 p-6 hover:shadow-navy-md hover:-translate-y-1 transition-all group"
            >
              {/* Logo */}
              <div className="flex items-center gap-3 mb-4">
                <div
                  className="w-12 h-12 rounded-xl flex items-center justify-center font-black text-white text-xl shadow-sm group-hover:scale-105 transition-transform"
              ...
              <div className="flex flex-wrap gap-1.5">
                {p.apis.map((api) => (
                  <span
                    key={api}
                    className="text-[10px] font-semibold px-2 py-1 rounded-lg border border-navy/10 text-navy/60"
