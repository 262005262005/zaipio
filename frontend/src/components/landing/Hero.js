...
function Hero() {
    const dashRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
    ...
    return (<section className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden pt-24 pb-16 bg-gradient-to-b from-[#041a2e] via-navy to-navy">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-14">
          <a href="/signup" className="w-full sm:w-auto bg-gradient-to-r from-accent to-sky-600 text-white font-bold text-base px-8 py-4 rounded-2xl shadow-[0_0_30px_rgba(14,165,233,0.5)] hover:brightness-110 hover:-translate-y-0.5 transition-all active:translate-y-0 flex items-center justify-center gap-2">
            <span>Start Free — No Card Needed</span>
            <span>→</span>
          </a>
          <a href="#features" className="w-full sm:w-auto bg-white/10 border border-white/20 text-white font-semibold text-base px-8 py-4 rounded-2xl hover:bg-white/20 transition-all backdrop-blur-sm">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 max-w-4xl mx-auto mb-16">
          {AGENTIC_PILLS.map((p) => (<div key={p.label} className="bg-white/[0.05] border border-white/10 p-3 rounded-2xl backdrop-blur-sm text-left">
        ...
        <div ref={dashRef} className="relative mx-auto max-w-5xl transition-transform duration-200 ease-out" style={{ transform: 'perspective(1200px) rotateY(-2deg) rotateX(2deg)' }}>
