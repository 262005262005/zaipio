    ...
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 flex flex-col md:flex-row items-center justify-between gap-4">
        <input
          ...
          className="w-full md:w-96 bg-slate-900 border border-slate-800 rounded-xl px-4 py-2 text-sm text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-red-500"
        />
          ...
          {['All', 'Free', 'Starter', 'Growth', 'Enterprise'].map((p) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                planFilter === p ? 'bg-red-600 text-white' : 'bg-slate-900 text-slate-400 hover:bg-slate-800'
              }`}
            >
              {p}
            </button>
              ...
              <tr key={s.id} className="hover:bg-slate-900/50 transition-colors">
                ...
                <td className="p-4 text-center">
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                    s.status === 'Active' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'
                  }`}>
                    {s.status}
                  </span>
                </td>
