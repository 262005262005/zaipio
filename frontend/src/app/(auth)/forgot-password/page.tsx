        ...
        <div className="max-w-md w-full mx-auto">
              ...
              <form onSubmit={handleRequest} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-navy/20 rounded-xl px-4 py-3 text-navy text-sm placeholder-navy/35 outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all"
                  />
                </div>
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-navy text-white font-bold text-sm py-3.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm disabled:opacity-60 flex items-center justify-center gap-2"
                >
                ...
                <div className="flex gap-3 justify-center">
                  {otp.map((digit, i) => (
                    <input
                      ...
                      className="w-11 h-14 text-center text-navy text-xl font-black border-2 border-navy/20 rounded-xl outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all bg-white"
                    />
                  ))}
                </div>
                <button
              ...
              <form onSubmit={handleReset} className="space-y-4">
                <div>
                  ...
                  {newPassword && (
                    <div className="flex gap-1 mt-2">
                      {[1,2,3,4].map((i) => (
                        <div key={i} className={`h-1 flex-1 rounded-full transition-all ${newPassword.length >= i * 3 ? i <= 1 ? 'bg-red-400' : i <= 2 ? 'bg-amber-400' : i <= 3 ? 'bg-accent' : 'bg-green-500' : 'bg-navy/10'}`} />
                      ))}
                    </div>
                  )}
                </div>
                <div>
                  ...
                  <input
                    ...
                    className={`w-full border rounded-xl px-4 py-3 text-navy text-sm placeholder-navy/35 outline-none focus:ring-2 focus:ring-accent/20 transition-all ${confirmPassword && newPassword === confirmPassword ? 'border-green-400' : 'border-navy/20 focus:border-accent'}`}
                  />
          ...
          {step === 'done' && (
            <div className="text-center py-8">
              ...
              <a
                href="/login"
                className="inline-block bg-navy text-white font-bold text-sm px-8 py-3.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm"
              >
                Back to Sign in
              </a>
            </div>
