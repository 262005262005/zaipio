...
function OnboardingPage() {
    ...
    const [selectedPlatform, setSelectedPlatform] = (0, react_1.useState)('shopify');
    const [connecting, setConnecting] = (0, react_1.useState)(false);
    const [connected, setConnected] = (0, react_1.useState)(false);
    const [alerts, setAlerts] = (0, react_1.useState)({ whatsapp: true, email: true, telegram: false, lowStockThreshold: '10' });
    ...
    return (<div className="min-h-screen bg-white font-outfit flex flex-col">
      ...
      <div className="border-b border-navy/10 px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden sm:flex items-center gap-1">
          {STEPS.map((s, i) => (<div key={s.id} className="flex items-center">
              <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${step === s.id
                ? 'bg-navy text-white'
                : step > s.id
                    ? 'text-green-600'
                    : 'text-navy/30'}`}>
              ...
              {i < STEPS.length - 1 && (<div className={`w-6 h-0.5 rounded-full mx-0.5 transition-all ${step > s.id ? 'bg-navy' : 'bg-navy/15'}`}/>)}
            </div>))}
        </div>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="max-w-2xl w-full">
          ...
          {step === 1 && (<div className="text-center">
            ...
            ].map((item) => (<div key={item.title} className="bg-navy/[0.03] border border-navy/10 rounded-2xl p-4 text-center">
              ...
              <button onClick={() => setStep(2)} className="bg-navy text-white font-bold text-base px-10 py-4 rounded-2xl hover:bg-navy-800 transition-all shadow-navy-md hover:-translate-y-0.5">
                Let's start →
              </button>
            </div>)}
          ...
          {step === 2 && (<div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                {PLATFORMS.map((p) => (<button key={p.id} onClick={() => setSelectedPlatform(p.id)} className={`text-left p-5 rounded-2xl border-2 transition-all hover:-translate-y-0.5 ${selectedPlatform === p.id
                    ? 'border-navy shadow-navy-md'
                    : 'border-navy/15 hover:border-navy/30'}`} style={{ background: selectedPlatform === p.id ? p.bg : 'white' }}>
                    <div className="flex items-start justify-between mb-3">
                      <div className="w-11 h-11 rounded-xl flex items-center justify-center text-white font-black text-lg" style={{ background: p.color }}>
                        {p.letter}
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        {p.recommended && (<span className="text-[10px] font-black bg-accent text-white px-2 py-0.5 rounded-full">
                            START HERE
                          </span>)}
                        {selectedPlatform === p.id && (<div className="w-5 h-5 bg-navy rounded-full flex items-center justify-center">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => setStep(3)} className="flex-1 bg-navy text-white font-bold text-sm py-3.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm">
          ...
          {step === 3 && (() => {
            ...
            return (<div className="text-center">
                <div className="w-20 h-20 rounded-3xl flex items-center justify-center text-white font-black text-4xl mx-auto mb-6 shadow-navy-md" style={{ background: p.color }}>
                  {p.letter}
                </div>
                ...
                {!connected ? (<>
                    ...
                    <div className="bg-navy/[0.03] border border-navy/10 rounded-2xl p-5 mb-8 text-left max-w-sm mx-auto">
                      ...
                      <ul className="space-y-2">
                        {['Read your orders', 'Read inventory levels', 'Update stock quantities', 'Read payment/settlement data'].map((item) => (<li key={item} className="flex items-center gap-2 text-navy/65 text-sm">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 justify-center">
                      ...
                      <button onClick={handleConnect} disabled={connecting} className="bg-navy text-white font-bold text-sm px-8 py-3.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm disabled:opacity-60 flex items-center gap-2 justify-center" style={{ background: connecting ? undefined : p.color }}>
                        {connecting ? (<><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"/> Connecting…</>) : (<>Authorize {p.name} →</>)}
                      </button>
                    </div>
                  </>) : (<>
                    ...
                    <button onClick={() => setStep(4)} className="bg-navy text-white font-bold text-sm px-8 py-3.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm">
                      Set up alerts →
                    </button>
                  </>)}
              </div>);
          ...
          {step === 4 && (<div>
              ...
              <div className="space-y-4 mb-8">
                ...
                <div className={`p-5 rounded-2xl border-2 transition-all ${alerts.whatsapp ? 'border-navy bg-navy/[0.03]' : 'border-navy/15'}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setAlerts(a => ({ ...a, whatsapp: !a.whatsapp }))} className={`w-12 h-6 rounded-full transition-all relative ${alerts.whatsapp ? 'bg-navy' : 'bg-navy/20'}`}>
                ...
                <div className={`p-5 rounded-2xl border-2 transition-all ${alerts.email ? 'border-navy bg-navy/[0.03]' : 'border-navy/15'}`}>
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => setAlerts(a => ({ ...a, email: !a.email }))} className={`w-12 h-6 rounded-full transition-all relative ${alerts.email ? 'bg-navy' : 'bg-navy/20'}`}>
                ...
                <div className="p-5 rounded-2xl border-2 border-navy/15">
                  ...
                  <div className="flex items-center gap-3">
                    <input type="number" min="1" max="999" value={alerts.lowStockThreshold} onChange={(e) => setAlerts(a => ({ ...a, lowStockThreshold: e.target.value }))} className="w-24 border border-navy/20 rounded-xl px-4 py-2.5 text-navy font-bold text-sm outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 text-center"/>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => setStep(5)} className="flex-1 bg-navy text-white font-bold text-sm py-3.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm">
                  Save & Continue →
                </button>
              </div>
            </div>)}
          ...
          {step === 5 && (<div className="text-center">
            ...
            ].map((item) => (<div key={item.label} className="bg-navy/[0.03] border border-navy/10 rounded-2xl p-4">
                    <div className="text-2xl mb-2">{item.icon}</div>
              ...
              <a href="/dashboard" className="inline-block bg-navy text-white font-bold text-base px-10 py-4 rounded-2xl hover:bg-navy-800 transition-all shadow-navy-md hover:-translate-y-0.5 mb-4">
                Open Dashboard →
              </a>
