...
function AdminSidebar() {
    ...
    return (<aside className="w-64 shrink-0 bg-slate-950 text-white flex flex-col h-screen overflow-y-auto border-r border-slate-800">
      {/* Header Badge */}
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        ...
        {ADMIN_NAV.map((item) => {
            ...
            return (<a key={item.href} href={item.href} className={`flex items-center justify-between px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all ${active
