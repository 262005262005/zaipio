...
function AdminDashboardPage() {
    return (<div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {METRICS.map((m) => (<div key={m.label} className="bg-slate-950 p-5 rounded-2xl border border-slate-800 space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-300">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {RECENT_SELLERS.map((s) => (<tr key={s.id} className="hover:bg-slate-900/50 transition-colors">
                    <td className="p-3">
        ...
        <div className="bg-slate-950 rounded-2xl border border-slate-800 p-6 space-y-4">
          ...
          <div className="space-y-3">
            {SYSTEM_HEALTH.map((item) => (<div key={item.service} className="p-3 rounded-xl bg-slate-900/80 border border-slate-800/80 space-y-1">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-bold text-slate-200">{item.service}</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${item.status === 'Healthy' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400'}`}>
                    {item.status}
                  </span>
                </div>
