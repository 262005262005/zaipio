...
exports.default = AdminSystemHealthPage;
const API_ENDPOINTS = [
      ...
      <div className="bg-slate-950 rounded-2xl border border-slate-800 overflow-hidden">
        ...
        <table className="w-full text-left text-sm text-slate-300">
          ...
          <tbody className="divide-y divide-slate-800/60">
            {API_ENDPOINTS.map((api) => (<tr key={api.name} className="hover:bg-slate-900/50 transition-colors">
                ...
                <td className="p-4 text-right">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${api.status === 'Operational' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400 animate-pulse'}`}>
                    {api.status}
                  </span>
                </td>
              </tr>))}
