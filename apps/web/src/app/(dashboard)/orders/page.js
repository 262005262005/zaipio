...
function OrdersPage() {
    ...
    const [density, setDensity] = (0, react_1.useState)('comfortable');
    const fetchOrders = async () => {
        setLoading(true);
        try {
            const res = await api_client_1.zaipioApi.getOrders();
            setOrders(res.data);
    ...
    return (<div className="space-y-5 relative text-left">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-black text-navy">Unified Orders</h1>
            {apiConnected && (<span className="flex items-center gap-1.5 text-[10px] font-mono font-bold bg-green-100 text-green-700 px-2.5 py-1 rounded-full border border-green-300">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="bg-navy/5 border border-navy/10 rounded-xl p-1 flex items-center gap-1">
            <button onClick={() => setDensity('comfortable')} className={`text-xs font-semibold px-2.5 py-1 rounded-lg transition-all ${density === 'comfortable' ? 'bg-white shadow text-navy' : 'text-navy/50'}`} title="Comfortable Row View">
              ☰ Comfortable
            </button>
            <button onClick={() => setDensity('compact')} className={`text-xs font-semibold px-2.5 py-1 rounded-lg transition-all ${density === 'compact' ? 'bg-white shadow text-navy' : 'text-navy/50'}`} title="Dense Compact View">
              ☵ Compact
            </button>
          </div>
          <button onClick={() => fetchOrders()} className="bg-navy text-white font-semibold text-xs px-4 py-2.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm flex items-center gap-1.5">
            <span>🔄</span> Sync API
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        ].map((s) => (<div key={s.label} className="bg-white border border-navy/10 rounded-xl p-3.5 shadow-sm">
      ...
      <div className="bg-white border border-navy/10 rounded-2xl p-4 shadow-sm flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-2 bg-navy/[0.04] border border-navy/10 rounded-xl px-3 py-2 w-full md:w-64">
          ...
          <input type="text" placeholder="Search order ID, product..." value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 bg-transparent text-navy text-sm placeholder-navy/35 outline-none"/>
        </div>
        <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto">
          {STATUSES.map((s) => (<button key={s} onClick={() => setStatusFilter(s)} className={`text-xs font-semibold px-3 py-1.5 rounded-xl transition-all whitespace-nowrap ${statusFilter === s ? 'bg-navy text-white shadow-sm' : 'bg-navy/5 text-navy/60 hover:bg-navy/10'}`}>
              {s}
            </button>))}
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-navy/5">
              {filtered.map((o) => (<tr key={o.id} onClick={() => setSelectedOrder(o)} className={`hover:bg-slate-50/80 transition-colors cursor-pointer group ${density === 'compact' ? 'py-1.5' : 'py-3.5'}`}>
                  <td className="p-4" onClick={(e) => e.stopPropagation()}>
                    <input type="checkbox" className="accent-navy"/>
                  </td>
                  <td className="p-4">
                    <div className="flex items-center gap-2">
                      <div className="w-6 h-6 rounded-lg flex items-center justify-center text-white font-black text-xs shrink-0 shadow-sm" style={{ background: o.platformColor || '#0EA5E9' }}>
                        {o.platform}
                      </div>
                      <div>
      ...
      {selectedOrder && (<div className="fixed inset-0 z-50 bg-navy/40 backdrop-blur-sm flex justify-end" onClick={() => setSelectedOrder(null)}>
          <div className="w-full max-w-md bg-white h-full shadow-2xl p-6 overflow-y-auto space-y-6 animate-slide-right text-left" onClick={(e) => e.stopPropagation()}>
            {/* Drawer Header */}
            <div className="flex items-center justify-between border-b border-navy/10 pb-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="w-5 h-5 rounded flex items-center justify-center text-white font-black text-[10px]" style={{ background: selectedOrder.platformColor || '#0EA5E9' }}>
              ...
              <button onClick={() => setSelectedOrder(null)} className="w-8 h-8 rounded-full bg-navy/5 hover:bg-navy/10 flex items-center justify-center text-navy font-bold text-sm">
                ✕
              </button>
            </div>
