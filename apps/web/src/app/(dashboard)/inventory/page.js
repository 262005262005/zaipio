...
function InventoryPage() {
    ...
    const [inventory, setInventory] = (0, react_1.useState)(MOCK_INVENTORY);
    ...
    const fetchInventory = async () => {
        try {
            const res = await api_client_1.zaipioApi.getInventory();
    ...
    const handleSyncAll = async () => {
        setLoading(true);
        try {
            await api_client_1.zaipioApi.updateStock('ALL_SKUS', 50);
    ...
    const filtered = inventory.filter(item => {
        ...
        if (filter === 'Critical/Low')
            return matchesSearch && (item.status === 'Critical' || item.status === 'Low Stock' || item.status === 'Out of Stock');
    ...
    return (<div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button disabled={loading} onClick={handleSyncAll} className="bg-navy/5 hover:bg-navy/10 text-navy font-semibold px-4 py-2.5 rounded-xl text-sm transition-all flex items-center gap-2">
            {loading ? (<span className="w-4 h-4 border-2 border-navy/30 border-t-navy rounded-full animate-spin"/>) : (<span>🔄</span>)}
            Sync All Now
          </button>
          <button onClick={() => {
      ...
      <div className="bg-white p-4 rounded-2xl border border-navy/5 shadow-navy-sm flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="relative w-full md:w-96">
          <input type="text" placeholder="Search by SKU name or code..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full bg-slate-50 border border-navy/10 rounded-xl px-4 py-2.5 text-sm text-navy placeholder:text-navy/40 focus:outline-none focus:border-accent"/>
        </div>
        <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto">
          {['All', 'Critical/Low', 'In Stock', 'Out of Stock'].map((t) => (<button key={t} onClick={() => setFilter(t)} className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all whitespace-nowrap ${filter === t ? 'bg-navy text-white shadow-sm' : 'bg-slate-100 text-navy/60 hover:bg-slate-200'}`}>
              {t}
            </button>))}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-navy/5 shadow-navy-sm overflow-hidden">
        <table className="w-full text-left text-sm text-navy">
          ...
          <tbody className="divide-y divide-navy/5">
            {filtered.map((item) => (<tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="p-4">
                ...
                <td className="p-4">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${item.status === 'In Stock' ? 'bg-green-100 text-green-700' :
