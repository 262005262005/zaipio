...
function PaymentsPage() {
    ...
    const fetchPayments = async () => {
        try {
            const res = await api_client_1.zaipioApi.getPaymentReconciliation();
    ...
    return (<div className="p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={handleExportCSV} className="bg-navy hover:bg-navy/90 text-white font-semibold px-4 py-2.5 rounded-xl text-sm shadow-navy-sm transition-all flex items-center gap-2">
      ...
      <div className="bg-white rounded-2xl border border-navy/5 shadow-navy-sm overflow-hidden">
        ...
        <table className="w-full text-left text-sm text-navy">
          ...
          <tbody className="divide-y divide-navy/5">
            {transactions.map((t) => (<tr key={t.id} className="hover:bg-slate-50/50 transition-colors">
                <td className="p-4">
                ...
                <td className="p-4 text-center">
                  <span className={`px-2.5 py-1 rounded-full text-xs font-bold ${t.status === 'Settled' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                    {t.status}
                  </span>
                </td>
              </tr>))}
