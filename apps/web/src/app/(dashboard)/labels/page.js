...
function LabelsPage() {
    const [tab, setTab] = (0, react_1.useState)('auto');
    const [dragOver, setDragOver] = (0, react_1.useState)(false);
    const [labels, setLabels] = (0, react_1.useState)(MOCK_LABELS);
    ...
    const fetchLabels = async () => {
        try {
            const res = await api_client_1.zaipioApi.getDailyLabelBatch();
    ...
    const handleCropLabels = async () => {
        setLoading(true);
        try {
            await api_client_1.zaipioApi.cropLabelsPdf({ cropRatio: '1x2', sort: 'SKU' });
    ...
    return (<div className="space-y-5">
      {/* Toast Notification */}
      {notification && (<div className="fixed bottom-5 right-5 bg-navy text-white text-xs font-semibold px-4 py-3 rounded-xl shadow-2xl flex items-center gap-2 z-50 animate-bounce">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          <button id="labels-download-all" onClick={() => {
          ...
          <button id="labels-print-all" onClick={() => {
      ...
      <div className="flex gap-1 bg-navy/[0.05] p-1 rounded-xl w-fit">
        ...
        ].map((t) => (<button key={t.id} onClick={() => setTab(t.id)} className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all ${tab === t.id ? 'bg-white text-navy shadow-navy-sm' : 'text-navy/50 hover:text-navy'}`}>
            {t.label}
          </button>))}
      </div>

      {tab === 'auto' && (<div className="bg-white rounded-2xl border border-navy/10 shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody className="divide-y divide-navy/[0.05]">
                {labels.map((l) => (<tr key={l.id} className="hover:bg-navy/[0.02] transition-colors group">
                    <td className="px-5 py-3.5">
                      <input type="checkbox" className="accent-navy"/>
                    </td>
                    <td className="px-3 py-3.5">
                      <div className="flex items-center gap-2">
                        <div className="w-5 h-5 rounded-md flex items-center justify-center text-white font-black text-[9px] shrink-0" style={{ background: l.platformColor || '#072946' }}>
                          {l.platform}
                        </div>
                        <div>
                    ...
                    <td className="px-5 py-3.5">
                      {l.status === 'Generating' ? (<span className="flex items-center gap-1.5 text-blue-600 text-[10px] font-bold">
                          <span className="w-3 h-3 border-2 border-blue-300 border-t-blue-600 rounded-full animate-spin"/>
                          Generating
                        </span>) : (<span className={`text-[10px] font-bold px-2.5 py-1 rounded-full ${STATUS_COLORS[l.status] || 'bg-slate-100 text-slate-700'}`}>
                          {l.status}
                        </span>)}
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        {l.status === 'Ready' && (<>
                            <button id={`label-download-${l.id}`} onClick={() => {
                        ...
                        setTimeout(() => setNotification(null), 3000);
                            ...
                            <button id={`label-print-${l.id}`} onClick={() => {
      ...
      {tab === 'manual' && (<div className="space-y-4">
          {/* Drop zone */}
          <div className={`border-2 border-dashed rounded-2xl p-12 text-center transition-all cursor-pointer ${dragOver ? 'border-accent bg-accent/5' : 'border-navy/20 hover:border-navy/40 bg-white'}`} onDragOver={(e) => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={(e) => { e.preventDefault(); setDragOver(false); handleCropLabels(); }} onClick={() => document.getElementById('label-file-input')?.click()} id="label-drop-zone">
            ...
            <input id="label-file-input" type="file" accept=".pdf" className="hidden" onChange={() => handleCropLabels()}/>
          </div>
          ...
          <div className="bg-white border border-navy/10 rounded-2xl p-5 shadow-sm">
            ...
            <button disabled={loading} onClick={handleCropLabels} className="mt-4 bg-navy text-white font-semibold text-sm px-6 py-2.5 rounded-xl hover:bg-navy-800 transition-all shadow-navy-sm w-full flex items-center justify-center gap-2">
              {loading ? (<>
