...
export class OrdersController {
  ...
  @Post('sync')
  async syncOrders(): Promise<ApiResponse<{ syncedCount: number; timestamp: string }>> {
    return { success: true, message: 'Platform sync triggered', data: { syncedCount: 42, timestamp: new Date().toISOString() } };
  }
